# Training Image-based ViT model, with leave-one-patient validation to evaluate the performance
import os
import cv2
import sys
import json
...
from ViT_model import ViT_MLP_gene
from CNN_training_function import setup_seed, train, valid, test, cal_gene_corr, test_spatial_visual
...
for run in range(runs):
    ...
    train_patch_dataset = patch_train_data(root=patch_root_path, tissue_list = tissue_list, exclude_tissue= tissue_name, transform=patch_transform)
    ...
    model = ViT_MLP_gene(attn_heads=attn_heads,dim_head=dim_head,hidden_features=hidden_features,gene_dim=num_gene,mlp_hidden_list=mlp_hidden).to(device)
