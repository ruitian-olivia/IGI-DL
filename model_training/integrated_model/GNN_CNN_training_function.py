...
def train_sigma(model,train_loader,optimizer,sigma_loss,sigma_inverse,device):
    model.train()

    sigma_inverse = sigma_inverse.to(device)
    ...
    for data in train_loader:
        ...
        optimizer.zero_grad()
        output = model(data)
        loss = sigma_loss(output, data.y, sigma_inverse)
        loss.backward()
        loss_all += data.num_graphs * loss.item()
        optimizer.step()
    return loss_all / len(train_loader.dataset)

def valid_sigma(model,loader,sigma_loss,sigma_inverse,device):
    model.eval()
    # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    loss = 0.
    ...
    for data in loader:
        ...
        loss_mean = sigma_loss(output, data.y, sigma_inverse)
        loss += data.num_graphs * loss_mean.item()
...
def test_sigma(model,loader,sigma_loss,sigma_inverse,device):
    model.eval()
    # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    loss_mse = 0.
    loss_sigma = 0. 
    ...
    for data in loader:
        ...
        loss_mean_mse = F.mse_loss(output, data.y, reduction='mean')
        loss_mse += data.num_graphs * loss_mean_mse.item()

        loss_mean_sigma = sigma_loss(output, data.y, sigma_inverse)
        loss_sigma += data.num_graphs * loss_mean_sigma.item()
    ...
    return loss_mse / len(loader.dataset), loss_sigma / len(loader.dataset), label, pred, x_coor, y_coor
