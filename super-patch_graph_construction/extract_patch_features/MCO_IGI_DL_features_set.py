...
model_name = 'IGI_DL_MCO'

graph_pt_root_path = '../preprocessed_MCO/MCO_filtered_graph_pt'
patient_list = os.listdir(graph_pt_root_path)
patient_list.sort()

set_num = 200
patient_set_list = [patient_list[i:i + set_num] for i in range(0, len(patient_list), set_num)]
patient_set = patient_set_list[set_id]
print("len(patient_set):", len(patient_set))
...
for patient_name in patient_set:
    ...
    try:
        ...
        print("Error in patient:", patient_name)
        continue
