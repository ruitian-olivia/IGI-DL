...
READ_svs_path = 'svs_path/TCGA_READ_svs_path.csv'
READ_svs_data = pd.read_csv(READ_svs_path, index_col=0)

READ_svs_data['st_size'] = READ_svs_data.apply(lambda a : os.stat(a['image_path']).st_size, axis=1)
READ_svs_data.sort_values(by="st_size", inplace=True, ascending=True)
READ_svs_sort = READ_svs_data.reset_index(drop=True)
READ_svs_sort.to_csv('svs_path/TCGA_READ_svs_size_sort.csv')
READ_svs_files = READ_svs_sort['image_path'].tolist()
print("len(READ_svs_files):", len(READ_svs_files))
...
answer = pool_obj.map(extract_100microns, READ_svs_files)