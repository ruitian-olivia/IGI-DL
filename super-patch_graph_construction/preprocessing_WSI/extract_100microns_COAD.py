...
COAD_svs_path = 'svs_path/TCGA_COAD_svs_path.csv'
COAD_svs_data = pd.read_csv(COAD_svs_path, index_col=0)

COAD_svs_data['st_size'] = COAD_svs_data.apply(lambda a : os.stat(a['image_path']).st_size, axis=1)
COAD_svs_data.sort_values(by="st_size", inplace=True, ascending=True)
COAD_svs_sort = COAD_svs_data.reset_index(drop=True)
COAD_svs_sort.to_csv('svs_path/TCGA_COAD_svs_size_sort.csv')
COAD_svs_files = COAD_svs_sort['image_path'].tolist()
print("len(COAD_svs_files):", len(COAD_svs_files))
...
answer = pool_obj.map(extract_100microns, COAD_svs_files)