...
from surv_models.Modified_GAT import GATConv as GATConv
from torch_geometric.nn import GraphSizeNorm

from surv_models.model_utils import weight_init
from surv_models.model_utils import decide_loss_type

from surv_models.pre_layer_surv import preprocess
from surv_models.post_layer_surv import postprocess
...
class CRC_GAT_surv(torch.nn.Module):

    def __init__(self, dropout_rate, dropedge_rate, Argument):
        super(CRC_GAT_surv, self).__init__()
        torch.manual_seed(12345)
        self.Argument = Argument
        ...
        self.residual = Argument.residual_connection
        self.norm_type = Argument.norm_type
        self.clinical_num = Argument.clinical_num
        ...
        self.postprocess = postprocess(dim * self.heads_num, self.layer_num, dim * self.heads_num, (Argument.MLP_layernum-1), dropout_rate)
        self.risk_prediction_layer = nn.Linear(self.clinical_num + self.postprocess.postlayernum[-1], 1, bias=False)
    ...
    def forward(self, data, edge_mask=None, Interpretation_mode=False):
        ...
        gender = data.gender.unsqueeze(1)
        encoded_stage = F.one_hot(data.stage.to(torch.int64), num_classes=5)[:, 2:]
        cancer_type = data.cancer_type.unsqueeze(1)
        
        clinical_concat =  torch.cat([age, gender, encoded_stage, cancer_type], dim=1)
        ...
        postprocessed_output = self.postprocess(x_concat, data.batch)
        feature_concat = torch.cat((clinical_concat, postprocessed_output), 1)
        risk = self.risk_prediction_layer(feature_concat)
...
class CRC_GAT_surv_cli(torch.nn.Module):

    def __init__(self, dropout_rate, dropedge_rate, Argument):
        super(CRC_GAT_surv_cli, self).__init__()
        torch.manual_seed(12345)
        self.Argument = Argument
    ...
    def forward(self, data, edge_mask=None, Interpretation_mode=False):
        ...
        age = data.age.unsqueeze(1)
        gender = data.gender.unsqueeze(1)
        stage = data.stage.unsqueeze(1)
        cancer_type = data.cancer_type.unsqueeze(1)
        
        clinical_concat =  torch.cat([age, gender, stage, cancer_type], dim=1)
...
class BRCA_GAT_surv_cli(torch.nn.Module):

    def __init__(self, dropout_rate, dropedge_rate, Argument):
        super(BRCA_GAT_surv_cli, self).__init__()
        torch.manual_seed(12345)
        self.Argument = Argument
    ...
    def forward(self, data, edge_mask=None, Interpretation_mode=False):
        ...
        clinical_concat =  torch.cat([age, stage], dim=1)
