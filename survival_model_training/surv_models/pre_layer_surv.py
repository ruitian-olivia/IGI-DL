...
class preprocess(torch.nn.Module):
    ...
    def __init__(self, argument):
        ...
        # 节点特征，Linear layer到 2*50=100维度
        self.prelayer = Linear(argument.node_feature_num, argument.attention_head_num * argument.initial_dim)
        # 边特征维度映射到50维
        self.edge_position_embedding = nn.Embedding(11, 50)
        self.edge_angle_embedding = nn.Embedding(11, 50)
        self.with_edge = argument.with_distance
        self.simple_distance = argument.simple_distance

    def reset_parameters(self):

        self.prelayer.apply(weight_init)
    ...
    def forward(self, data, edge_mask=None):
        ...
        preprocessed_data = self.prelayer(preprocessed_data)
