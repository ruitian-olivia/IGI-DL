...
def Train(Argument):
    ...
    READ_csv_path = '../super-patch_graph_construction/preprocessed_WSI/surv_csv/READ_surv.csv'
    READ_df = pd.read_csv(READ_csv_path)
    
    COAD_csv_path = '../super-patch_graph_construction/preprocessed_WSI/surv_csv/COAD_surv.csv'
    ...
    gender_beta = np.zeros((runs,epochs))
    stage_beta = np.zeros((runs,epochs))
    type_beta = np.zeros((runs,epochs))
    ...
    for Fi in range(runs):
        ...
        for epoch in range(0, epochs):
            ...
            gender_beta[Fi,epoch] = clinical_weight[1]
            stage_beta[Fi,epoch] = clinical_weight[2]
            type_beta[Fi,epoch] = clinical_weight[3]         
        ...
        test_LOOV_cox_surv(model, val_loader, cox_loss)
        test_acc[Fi], test_surv_df = test_LOOV_cox_surv(model, val_loader, cox_loss)
