...
import sys
import argparse
from BRCA_surv_cli_train import Train
import torch

def Parser_main():
    parser = argparse.ArgumentParser(description="Deep cox analysis model")

    parser.add_argument("--model_name", default="BRCA_GAT_surv_clinical", help="Model name", type=str)
    ...
    parser.add_argument("--node_feature_num", default=69, help="Number of node features in constructed Super-node Graph", type=int)
    parser.add_argument("--clinical_num", default=2, help="Number of encoded clinical features", type=int)
